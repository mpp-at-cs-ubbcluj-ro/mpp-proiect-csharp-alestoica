...
namespace csharp_project.repository
{
    public class EmployeeDbRepository : IEmployeeRepository
    {
        ...
        {
            log.InfoFormat("entering FindOneByUsernameAndPassword with values: username={0}", username);
            var con = DbUtils.GetConnection(props);
            

            using (var comm = con.CreateCommand())
            {
                comm.CommandText = "select id, first_name, last_name from employees where username = @username and password = @password";
                ...
                using (var dataR = comm.ExecuteReader())
                {
                    if (dataR.Read())
                    {
                        ...
                        log.InfoFormat("exiting FindOneByUsernameAndPassword with value: {0}", employee);
                        return employee;
                    }
                }
            }

            log.InfoFormat("exiting FindOneByUsernameAndPassword with value: null");
            return null;
        }
